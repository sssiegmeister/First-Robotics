...
const tMUXSensor color = msensor_S3_3;
...
void deposit(){
	ClearTimer(T3);
	while(red<100&&time1[T3]<4000)
	{
		motor[winch]=100;
		HTCS2readRGB(color, red, green, blue);
		wait1Msec(1);
	}
	move(0);
	wait1Msec(5000);
	ClearTimer(T3);
	while(green<100&&time1[T3]<4000)
	{
		motor[winch]=-15;
		HTCS2readRGB(color, red, green, blue);
		wait1Msec(1);
...
task IRSweep1
{
	...
	while(time1[T2]<1000)
	{
		...
		distance = SensorValue[sonar];
		HTIRS2readAllACStrength(Seeker, acS1, acS2, acS3, acS4, acS5);
		if((acS4-acS3)<70&&(acS4+acS3)>30&&time1[T2]>1000)
		{
			PlaySound(soundBeepBeep);
			strafe(-25);
			wait1Msec(250);
